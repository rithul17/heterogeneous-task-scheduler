...
void cpu_task_initialize(float* input, float* output, int size) {
    ...
    for (int i = 0; i < size; i++) {
        input[i] = (float)(rand() % 10000) / 10.0f;
...
void execute_cpu_task(Task* task) {
    ...
    printf("CPU task %s completed in %.3f seconds\n", task->name, task->execution_time);
...
void execute_gpu_task(Task* task) {
    ...
    printf("GPU task %s completed in %.3f seconds\n", task->name, task->execution_time);
    task->state = TASK_STATE_COMPLETED;
}
...
void scheduler_run() {
    ...
    for (int i = 0; i < scheduler.task_count; i++) {
        Task* task = &scheduler.tasks[i];
        printf("%-2d | %-20s | %-4s | %7.30f | %s\n",
               task->id,
               task->name,
               task->type == TASK_TYPE_CPU ? "CPU" : "GPU",
               task->execution_time*100,
...
int main() {
    ...
    printf("Analysis results: Count=%.0f, Mean=%.2f, Variance=%.2f, Sum=%.2f\n",
